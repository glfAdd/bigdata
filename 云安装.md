...
```bash
# hadoop
$ wget hadoop https://mirrors.bfsu.edu.cn/apache/hadoop/common/hadoop-3.2.2/hadoop-3.2.2.tar.gz
...
<configuration>
    {#用于设置 hadoop 的默认文件系统, 由 URI 指定, HDFS 通过该属性确定 namenode 运行的主机和端口#}
    <property>
        <name>fs.defaultFS</name>
        <value>hdfs://centos01:8020</value>
    </property>
    {#Hadoop的默认临时路径. hadoop.tmp.dir是hadoop文件系统依赖的基础配置，很多路径都依赖它。它默认的位置是在/tmp/{$user}下面，但是在/tmp路径下的存储是不安全的，因为linux一次重启，文件就可能被删除#}
    <property>
